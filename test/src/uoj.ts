    ...
    public async submit(problemID: string, code: string, language: string) {
        ...
        try {
            ...
            const runID: string = await page.evaluate(() => {
                const results = document.querySelector('table[class="table table-bordered table-hover table-striped table-text-center"]').lastElementChild.childNodes;
                // tslint:disable-next-line:no-shadowed-variable
                let runID: string = null;
                for (const result of results) {
                    if (result.childNodes[2].textContent === this.username) {
                        runID = result.childNodes[0].textContent;
                    }
                }
                return runID;
            });
